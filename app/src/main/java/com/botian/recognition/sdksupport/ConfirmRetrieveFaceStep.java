...
/** 1:N 场景手动(UI交互)打卡 */
public abstract class ConfirmRetrieveFaceStep extends AbsStep<StuffBox> {
    ...
    public ConfirmRetrieveFaceStep(StuffId<Collection<TrackedFace>> inputColorFacesId) {
    ...
    public ConfirmRetrieveFaceStep(InputProvider inputFacesProvider) {
    ...
    protected boolean onProcess(StuffBox stuffBox) {
        ...
        for (TrackedFace face : inColorFaces) {
            ...
            FaceForConfirm confirmedFace = new FaceForConfirm(face, MyApplication.tempUserName, faceBitmap);
